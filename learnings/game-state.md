...
now takes the expectation instead: the minimum over 8 samples is the un-doubled quantity - every
deterministic bonus the accessor applies survives, only the coin flip is gone - and
`getDoublingChance` turns it back into an average. When that getter refuses, and several
per-action chance getters in this adapter do refuse during enumeration
(`Mining.getRockGemChance` has never once succeeded), the mean of the samples stands in: still
unbiased, with an eighth of the variance, and the refusal is counted under
`candidates.doublingChance:<skillId>` rather than silently stripping the bonus.
