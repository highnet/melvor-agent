...
declare abstract class BaseManager
  ...
  implements
    ...
    EncodableObject,
    ActiveAction,
    INoWildCardGameEventEmitter<BaseCombatEvents>
{
  game: Game;
  abstract _events: Pick<Emitter<BaseCombatEvents>, 'emit'>;
  abstract on<Key extends keyof BaseCombatEvents>(
    type: Key,
    handler: Handler<BaseCombatEvents[Key]>,
  ): void;
  abstract off<Key extends keyof BaseCombatEvents>(
    type: Key,
    handler: Handler<BaseCombatEvents[Key]>,
  ): void;
   ...
   * @param posMult A multiplier for conditional modifiers that are positive
   */
  addAndActivateConditionalModifiers(
    originalSource: ModifierSource,
    conditionals: ConditionalModifier[],
    negMult?: number,
    posMult?: number,
