...
declare class RaidManager extends BaseManager {
  ...
  on: {
    <Key extends keyof BaseCombatEvents>(
      type: Key,
      handler: import('mitt').Handler<BaseCombatEvents[Key]>,
    ): void;
  ...
  off: {
    <Key extends keyof BaseCombatEvents>(
      type: Key,
      handler?: import('mitt').Handler<BaseCombatEvents[Key]> | undefined,
    ): void;
    (type: '*', handler: import('mitt').WildcardHandler<BaseCombatEvents>): void;
  };
  get media(): string;
  ...
  getItemChoices<T extends keyof RaidItemTypeMap>(
    category: T,
    count: number,
  ): RaidItemBeingSelected<RaidItemTypeMap[T]>[];
  createNewEnemy(): void;
