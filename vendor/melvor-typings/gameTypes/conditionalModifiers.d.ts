...
declare type CombatConditionData =
...
declare function checkEveryCondition<T>(
  condition: EveryCondition<T>,
  checkCondition: (condition: T) => boolean,
): boolean;
interface SomeCondition<T> {
  type: 'Some';
  conditions: T[];
}
declare function checkSomeCondition<T>(
  condition: SomeCondition<T>,
  checkCondition: (condition: T) => boolean,
): boolean;
declare type ConditionalModifierConditionData =
  | CombatConditionData
  | ItemInBankConditionData
  | ItemChargeConditionData
  | PotionUsedConditionData
  | EveryCondition<ConditionalModifierConditionData>
  | SomeCondition<ConditionalModifierConditionData>
...
declare class SomeConditionClass extends ConditionalModifierCondition {
  ...
  constructor(
    data: SomeCondition<ConditionalModifierConditionData>,
    game: Game,
    selfItem?: EquipmentItem,
  );
  checkIfMet(manager: BaseManager): boolean;
  isEquals(condition: ConditionalModifierCondition): boolean;
  _assignWrappedHandler(manager: BaseManager, handler: VoidFunction): void;
...
declare class EveryConditionClass extends ConditionalModifierCondition {
  ...
  conditions: ConditionalModifierCondition[];
  constructor(
    data: EveryCondition<ConditionalModifierConditionData>,
    game: Game,
    selfItem?: EquipmentItem,
  );
  checkIfMet(manager: BaseManager): boolean;
...
declare class ConditionalModifier implements SoftDataDependant<ConditionalModifierData> {
  ...
  static getConditionFromData(
    data: ConditionalModifierConditionData,
    game: Game,
    selfItem?: EquipmentItem,
  ): ConditionalModifierCondition;
}
