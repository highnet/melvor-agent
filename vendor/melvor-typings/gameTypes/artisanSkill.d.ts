...
declare abstract class ArtisanSkill<
  RecipeClass extends ArtisanSkillRecipe,
  DataType extends MasterySkillData,
  ProductType extends Item,
  Events extends SkillWithMasteryEvents = SkillWithMasteryEvents,
  ModificationDataType extends MasterySkillModificationData = MasterySkillModificationData,
> extends CraftingSkill<RecipeClass, DataType, Events, ModificationDataType> {
  /** Associated menu for the skill */
  abstract menu: ArtisanMenuElement;
  abstract selectionTabs: Map<
    SkillCategory,
    RecipeSelectionTabElement<RecipeClass, RecipeOptionElement<RecipeClass>>
  >;
...
declare class ArtisanSkillRenderQueue<
  ActionType extends ArtisanSkillRecipe,
> extends GatheringSkillRenderQueue<ActionType> {
...
declare abstract class CategorizedArtisanRecipe<
  CategoryType extends SkillCategory,
> extends ArtisanSkillRecipe {
  skill: AnySkill & SkillCategoryObject<CategoryType>;
  category: CategoryType;
  constructor(
    namespace: DataNamespace,
    data: CategorizedArtisanRecipeData,
    game: Game,
    skill: AnySkill & SkillCategoryObject<CategoryType>,
  );
...
interface SingleProductArtisanSkillRecipeModificationData
  extends CategorizedArtisanRecipeModificationData {
  productID?: string;
  baseQuantity?: number;
}
declare class SingleProductArtisanSkillRecipe<
  CategoryType extends SkillCategory,
> extends CategorizedArtisanRecipe<CategoryType> {
  get name(): string;
  get media(): string;
  product: AnyItem;
  baseQuantity: number;
  constructor(
    namespace: DataNamespace,
    data: SingleProductArtisanSkillRecipeData,
    game: Game,
    skill: Skill<any> & SkillCategoryObject<CategoryType>,
  );
