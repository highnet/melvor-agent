...
declare type ExprTestFunction = (
  exprString: string | number,
  type: ExprPrimaryType,
) => ExprTestResult;
...
declare const expressions: {
  ...
  getCombatEffectNumberTranspiler: (
    effect: CombatEffect,
  ) => IExprTranspiler<CombatEffectNumberFunc>;
  getInitialParamNumberTranspiler: (
    paramNames: string[],
  ) => IExprTranspiler<(initialParams: Record<string, number>) => number>;
