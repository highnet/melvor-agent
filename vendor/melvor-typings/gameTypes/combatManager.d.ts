...
declare class CombatManager
  extends BaseManager
  implements PassiveAction, IGameEventEmitter<CombatEvents>
{
  _events: import('mitt').Emitter<CombatEvents>;
  on: {
    <
      Key extends
        | keyof BaseCombatEvents
        | 'monsterKilled'
        | 'monsterDrop'
        | 'boneDrop'
        | 'monsterSpawned'
    ...
    >(
      type: Key,
      handler: import('mitt').Handler<CombatEvents[Key]>,
    ): void;
  ...
  off: {
    ...
    >(
      type: Key,
      handler?: import('mitt').Handler<CombatEvents[Key]> | undefined,
    ): void;
    (type: '*', handler: import('mitt').WildcardHandler<CombatEvents>): void;
  };
  player: Player;
