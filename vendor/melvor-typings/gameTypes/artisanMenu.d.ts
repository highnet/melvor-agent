declare class ArtisanMenuElement<ProductItem extends Item = Item>
  extends HTMLElement
  implements CustomElement
{
  ...
  updateChances(
    ...
    preserveCap: number,
    preserveSources: HTMLSpanElement[],
    doublingChance: number,
    doublingSources: HTMLSpanElement[],
  ): void;
  ...
  setRecipeDropdown(
    altRecipeIngredients: {
      items: AnyItemQuantity[];
      currencies: CurrencyQuantity[];
    }[],
    selectCallback: (recipeID: number) => VoidFunction,
    displayOrder?: number[],
  ): void;
}
declare class HerbloreArtisanMenuElement
  extends ArtisanMenuElement<PotionItem>
  implements CustomElement
{
  get $template(): string;
