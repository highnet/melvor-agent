...
interface NewSaveGame
  extends NumberSaveGame,
    BoolSaveGame,
    SerializableSaveGame,
    NestedSerializeableSaveGame,
    OtherSaveGame,
    ReconstructedSaveGame,
    Partial<OldSaveGame> {
  serialData?: DataReader;
  version: number;
}
declare type NewSaveKey = NumberKey | BoolKey | SerialKey | NestedKey | OtherKey | ReconKey;
type Serializer<T> = (saveVar: T) => number[];
type NestedSerializer<T> = (saveVar: T) => number[][];
type Deserializer<T> = (sData: number[], version: number) => T;
type NestedDeserializer<T> = (sData: number[][], version: number) => T;
...
declare const currentSaveVersion = SaveVersion.ItemClaimCount;
declare enum SaveVersion {
  ModProfiles = 83,
...
declare function testDiff<T>(
  diff: SaveKeyDiff<T>,
  vars: NumberDictionary<T[]>,
  version: number,
): void;
...
declare function deserializeItemStatsPortion(
  portion: StatsData,
  subData: number[],
  stats: ItemStat[],
  itemStatVer: number,
...
declare const deserializeItemStats: (
  sData: number[][],
  sVersion: number,
  idMap: NumericIDMap,
) => ItemStat[];
...
declare function getSaveFromString(
  saveString: string,
  idMap: NumericIDMap,
): {
...
declare function setSaveKeyToDefault<T extends keyof NewSaveGame>(
  saveGame: NewSaveGame,
  key: T,
): void;
/** Converts herblore bonuses from array */
declare function convertHerbloreBonusesFromArray(
  herbloreBonuses: OldHerbloreBonus[],
): OldHerbloreBonuses;
