...
declare abstract class Character
  implements EncodableObject, Serializable, INoWildCardGameEventEmitter<CharacterCombatEvents>
{
  manager: BaseManager;
  game: Game;
  abstract _events: Pick<Emitter<CharacterCombatEvents>, 'emit'>;
  abstract on<Key extends keyof CharacterCombatEvents>(
    type: Key,
    handler: Handler<CharacterCombatEvents[Key]>,
  ): void;
  abstract off<Key extends keyof CharacterCombatEvents>(
    type: Key,
    handler: Handler<CharacterCombatEvents[Key]>,
  ): void;
  ...
  onEffectApplicatorTrigger(
    trigger: CombatEffectApplicationTrigger,
  ...
  checkEffectApplicatorCondition(
