...
declare type PageSideBarItemOptions = Omit<
  NoElementConfig<SidebarItemConfig>,
  'name' | 'link' | 'onClick' | 'onRender'
> & {
  categoryID: string;
};
declare type PageSideBarSubItemOptions = Omit<
  NoElementConfig<SidebarSubitemConfig>,
  'link' | 'onClick' | 'onRender'
> & {
  categoryID: string;
  itemID: string;
};
