...
declare class NamespaceRegistry<T extends NamespacedObject> {
  ...
  getDummyObject(
    id: string,
    DummyObject: new (namespace: DataNamespace, localID: string, game: Game) => T,
    game: Game,
  ): T;
  ...
  forEachInNamespace(
    namespace: string,
    callbackfn: (value: T, id: string) => void,
  ): true | undefined;
  ...
  reduce<U>(
    callbackfn: (previousValue: U, currentValue: T, id: string, map: Map<string, T>) => U,
    initialValue: U,
  ): U;
  ...
  modifyQuantities(
    quantities: ItemQuantity<T>[],
    data: ItemQuantitiesModificationData,
  ): ItemQuantity<T>[];
