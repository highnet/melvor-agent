declare namespace Modding {
  ...
  type ClassPatches<T> = Map<
    keyof T,
    {
      mods: Set<Mod>;
      patcher: MethodPatch<T, AnyFunction> | PropertyPatch<T, any>;
    }
  >;
  interface PatchMap extends Map<Constructor, ClassPatches<any>> {
    get<T>(key: Constructor<T>): ClassPatches<T>;
  }

  interface MethodPatch<T, M extends AnyFunction> {
    before(patch: BeforePatch<T, M>): void;
    replace(patch: ReplacePatch<T, M>): void;
    after(patch: AfterPatch<T, M>): void;
  }

  interface PropertyPatch<T, P> {
    get(getter: GetterPatch<T, P>): void;
    set(setter: SetterPatch<T, P>): void;
    replace(getter: GetterPatch<T, P>, setter: SetterPatch<T, P>): void;
  }
  type GetterPatch<T, P> = (this: T, o: () => P) => P;
  ...
  type ReplacePatch<T, M extends AnyFunction> = (
    this: T,
    replacedMethod: (...args: Parameters<M>) => ReturnType<M>,
    ...args: Parameters<M>
  ) => ReturnType<M>;

  type MethodOrPropertyPatch<T, K extends keyof T> = T[K] extends AnyFunction
    ? MethodPatch<T, T[K]>
    : PropertyPatch<T, T[K]>;
...
type MethodName<T> = { [P in keyof T]-?: T[P] extends AnyFunction ? P : never }[keyof T];
type PropertyName<T> = { [P in keyof T]-?: T[P] extends AnyFunction ? never : P }[keyof T];
