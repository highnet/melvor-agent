...
declare class Player extends Character implements IGameEventEmitter<PlayerCombatEvents> {
  ...
  on: {
    <
      Key extends
        ...
        | 'summonTabletUsed'
        | 'soulPointsUsed'
        | keyof CharacterCombatEvents,
    >(
      type: Key,
      handler: import('mitt').Handler<PlayerCombatEvents[Key]>,
    ): void;
  ...
  off: {
    ...
    >(
      type: Key,
      handler?: import('mitt').Handler<PlayerCombatEvents[Key]> | undefined,
    ): void;
    (type: '*', handler: import('mitt').WildcardHandler<PlayerCombatEvents>): void;
  };
  get type(): string;
  ...
  checkSlayerItemEquipRestrictionsOnEquipSetChange(
    restrictions: CombatEquipmentRestrictions,
   ...
   */
  checkDamageTypeEquipRestrictions(
    restrictions: CombatEquipmentRestrictions,
   ...
   */
  checkSlayerItemEquipRestrictions(
    restrictions: CombatEquipmentRestrictions,
    removedItems: ItemQuantity<EquipmentItem>[],
  ...
  removeSummonCharge(
    slotID: EquipmentSlotIDs.Summon1 | EquipmentSlotIDs.Summon2,
    interval: number,
  ): void;
