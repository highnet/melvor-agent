...
The agent has **47 capabilities**. Each is a decision the planner can
...
| `claim_mastery_token` | Claim a Mastery Token into its skill mastery pool, which the sell list was otherwise offering to liquidate |
| `claim_township_task` | Claim a finished Township task |
| `compost_plot` | Compost a growing plot so the crop cannot die |
| `convert_from_township` | Trade town resources back for goods only the town can make, such as Herb Boxes |
| `convert_to_township` | Trade bank items to the town for resources it can build with |
| `create_dig_map` | Make a new map for a dig site — maps are consumable, and Archaeology has no candidates at all once the last one is spent |
...
| Township | `build_township`, `repair_township`, `repair_all_township`, `claim_township_task` |
| Cartography | `survey_hex`, `make_paper`, `travel_to_poi` |
| Archaeology | `excavate_dig_site`, with `create_dig_map`, `select_dig_map` and `select_dig_tool` |
